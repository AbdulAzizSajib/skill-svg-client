...
export default function SvgBrowseClient() {
  ...
  const query = useMemo<ISvgListQuery>(() => {
    ...
    return {
      page,
      limit,
      sortBy: (searchParams.get('sortBy') as ISvgListQuery['sortBy']) ?? 'title',
      sortOrder: (searchParams.get('sortOrder') as ISvgListQuery['sortOrder']) ?? 'asc',
      search: searchParams.get('search') || undefined,
      categoryId: searchParams.get('categoryId') || undefined,
      tag: searchParams.get('tag') || undefined,
      visibility: (searchParams.get('visibility') as ISvgListQuery['visibility']) ?? 'PUBLIC',
    };
  }, [searchParams]);
    ...
    <section className="space-y-6 max-w-6xl mx-auto py-4">
      <header className="flex flex-col items-center text-center gap-2 py-2">
        ...
        <p className="text-[15px] text-muted-foreground leading-relaxed max-w-xl">
          Browse, copy, and embed high-quality tech stack symbols from our public library — built
      ...
      {meta ? (
        <SvgPagination page={meta.page} totalPages={meta.totalPages} onChange={onPageChange} />
      ) : null}
        ...
        <div className="max-w-5xl mx-auto px-4 flex flex-col gap-8">
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            ...
            <div className="flex items-center gap-6 flex-wrap">
              <a
                href="https://github.com/AbdulAzizSajib"
                target="_blank"
                rel="noopener noreferrer"
